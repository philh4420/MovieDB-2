    ...
    const fetchCastDetails = async () => {
      try {
        const response = await axios.get(
          `https://api.themoviedb.org/3/person/${props.params.id}?api_key=${apiKey}&language=en-US&append_to_response=external_ids,homepage`
        );
    ...
    if (castDetails && castDetails.external_ids) {
      ...
      return socialMediaPlatforms.map((platform) => {
        ...
        if (socialMediaId) {
          ...
          return (
            <a key={name} href={url} target="_blank" rel="noopener noreferrer" className="cast-details__social-link">
  ...
  return (
    <div className="cast-details">
      ...
      <div className="cast-details__content">
        <div className="cast-details__info">
          <h1 className="cast-details__name">{name}</h1>
          <p className="cast-details__biography">{biography}</p>
          ...
          {known_for && (
            <div className="cast-details__known-for">
              <h2 className="cast-details__known-for-title">Known For:</h2>
                ...
                {known_for.map((movie) => (
                  <div key={movie.id} className="cast-details__known-for-movie">
                    ...
                    <div className="cast-details__known-for-movie-details">
                      <p className="cast-details__known-for-movie-title">{movie.title}</p>
                      <p className="cast-details__known-for-movie-character">{movie.character}</p>
                      <button
                        className="cast-details__known-for-movie-button"
                        onClick={() => {
          ...
          <div className="cast-details__personal-info">
            ...
            <a href={homepage} target="_blank" rel="noopener noreferrer" className="cast-details__homepage-link">
