...
const pageSize = 11; // Number of movies per page
...
const CastDetails = (props) => {
  ...
  const renderKnownForMovies = () => {
    if (castDetails && castDetails.movie_credits) {
      ...
      const totalMovies = movie_credits.cast.length;
      const totalPages = Math.ceil(totalMovies / pageSize);
      const startIndex = (currentMoviePage - 1) * pageSize;
      const endIndex = startIndex + pageSize;
      const moviesToShow = movie_credits.cast.slice(startIndex, endIndex);
