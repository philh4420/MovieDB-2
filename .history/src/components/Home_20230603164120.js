...
const Home = () => {
  ...
  const fetchUpcomingMovies = async (page) => {
    try {
      ...
      const response = await axios.get(
        `https://api.themoviedb.org/3/movie/upcoming?api_key=${apiKey}&language=en-US&primary_release_date.gte=${currentDate}&page=${page}`
      );
  ...
  const fetchUpcomingTVShows = async (page) => {
    try {
      const currentDate = new Date().toISOString().split('T')[0];
      const response = await axios.get(
        `https://api.themoviedb.org/3/tv/on_the_air?api_key=${apiKey}&language=en-US&first_air_date.gte=${currentDate}&page=${page}`
      );
