...
const Home = () => {
  ...
  const fetchData = async (url, setData, setTotalPages) => {
    try {
  ...
  useEffect(() => {
    fetchData('https://api.themoviedb.org/3/movie/upcoming', setUpcomingMovies, setTotalMoviePages);
    fetchData('https://api.themoviedb.org/3/tv/on_the_air', setUpcomingTVShows, setTotalTVShowPages);
  ...
  return (
    <div className="home">
      ...
      <ul className="home__list">
        {upcomingTVShows && upcomingTVShows.map((tvShow) => (
