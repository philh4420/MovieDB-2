...
if (!apiKey) {
  console.error('REACT_APP_API_KEY is not set in the environment variables');
}
...
const MovieDetails = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [details, castData, trailerData] = await Promise.all([
          fetchMovieDetails(props.params.id),
          fetchCast(props.params.id),
          fetchTrailers(props.params.id),
        ]);
