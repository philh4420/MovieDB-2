  ...
  useEffect(() => {
    const fetchTVShowDetails = async () => {
      try {
        ...
        if (externalIdsResponse.data.homepage) {
          externalLinks.push({ platform: 'homepage', icon: globe, id: externalIdsResponse.data.homepage });
        }
          ...
          <div>
            ...
            <div className="tv-show-details__external-links">
              {links.map((link) => (
                <a
                  key={link.platform}
                  href={link.platform === 'homepage' ? link.id : `https://www.${link.platform}.com/${link.id}`}
                  target="_blank"
