...
const TVShowDetails = () => {
  ...
  if (error) {
    return <div className="show-details__error">{error}</div>;
  }
  ...
  return (
    <div className="show-details">
      ...
      <div className="show-details__cast">
        <p>Cast:</p>
        <div className="show-details__cast-row">
          {cast.map((person) => (
            <div key={person.id} className="show-details__cast-item">
              <Link to={`/cast/${person.id}`}>
                <img
                  className="show-details__cast-image"
                  src={`https://image.tmdb.org/t/p/w200/${person.profile_path}`}
                  alt={person.name}
                />
              </Link>
              <p className="show-details__cast-name">{person.name}</p>
              <Link to={`/cast/${person.id}`}>
                <button className="show-details__cast-button">View Details</button>
              </Link>
            </div>
          ))}
        </div>
